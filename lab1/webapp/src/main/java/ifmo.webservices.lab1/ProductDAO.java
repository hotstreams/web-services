...
public class ProductDAO {
    private final String SELECT_ALL = "SELECT id, code, name, category, quantity, cost FROM PRODUCT";
    ...
    private List<Product> getSpecifiedProducts(final Map<String, String> args) {
        try (Connection conn = PostgresConnection.getConnection()) {
            PreparedStatement preparedStatement = conn.prepareStatement(SELECT_ALL + convertArgsToWhereClause(args));
            int objectIndex = 1;
